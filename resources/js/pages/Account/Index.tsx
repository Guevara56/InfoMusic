    ...
    <PublicLayout>
      ...
      <div style={{ maxWidth: 700, margin: '0 auto' }}>
        ...
        <Section title="Seguridad" icon={Shield}>
          ...
          {/* 2FA */}
          <div>
            <h3 style={{ fontSize: 13, fontWeight: 600, color: '#888', display: 'flex', alignItems: 'center', gap: 6, marginBottom: '1rem' }}>
              <KeyRound size={13} /> Autenticación de dos factores (2FA)
            </h3>

            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.8rem 1rem', background: '#0d0d18', border: '1px solid #2a2a3a', borderRadius: 8, marginBottom: '1rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                {twoFactorEnabled
                  ? <ShieldCheck size={16} color="#c8f050" />
                  : <ShieldBan size={16} color="#555" />
                }
                <span style={{ fontSize: 13, fontWeight: 500 }}>
                  {twoFactorEnabled ? '2FA activado' : '2FA desactivado'}
                </span>
              </div>
              <span style={{ fontSize: 11, fontWeight: 600, padding: '3px 10px', borderRadius: 20, background: twoFactorEnabled ? 'rgba(200,240,80,0.12)' : 'rgba(239,68,68,0.1)', color: twoFactorEnabled ? '#c8f050' : '#ef4444' }}>
            ...
            {twoFactorEnabled ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                <TwoFactorRecoveryCodes recoveryCodesList={recoveryCodesList} fetchRecoveryCodes={fetchRecoveryCodes} errors={twoFaErrors} />
                <Form {...disable.form()}>
                  {({ processing }) => (
                    <button type="submit" disabled={processing}
                      style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '10px 18px', background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: 8, color: '#ef4444', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
                      <ShieldBan size={14} /> Desactivar 2FA
                    </button>
                  )}
                </Form>
              </div>
              ...
              <div>
                ...
                {hasSetupData ? (
                  <button onClick={() => setShowSetupModal(true)} style={btnSecondary}>
                    <ShieldCheck size={14} /> Continuar configuración
                  </button>
                ) : (
                  <button
                    onClick={() => setShow2FAPasswordModal(true)}
                    style={btnSecondary}
                  >
                    <ShieldCheck size={14} />
                    Activar 2FA
